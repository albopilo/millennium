...
const deleteReservation = async (reason) => {
  ...
  const handleDeleteReservation = async (reasonInput = null) => {
   if (!isAdmin || !reservation) return;
  const reason = reasonInput || prompt("Please enter a reason for deleting this reservation:");
   if (!reason || reason.trim().length < 3) {
     alert("Please provide a reason (at least 3 characters).");
     return;
   }
   await deleteReservation(reason.trim());
 };
  ...
  const confirmDeleteReservation = async () => {
   ...
   await deleteReservation(deleteReason.trim());
   closeDeleteModal();
 };
...
async function logAction(actionOrObj, payload = {}) {
  ...
  // Subscribe to reservation logs for UI (render at bottom)
  useEffect(() => {
